...
public class BETA_SETTINGS{
    ...
    private void Update()
    {
        ...
        if((fps_Rigidbody.velocity.magnitude > 1.9f && !IsGrounded) || isFalling)
        {
            anim.SetBool("Falling", true);
            isFalling = true;
            head.localPosition = Vector3.Lerp(head.localPosition, new Vector3(0f, 0.6f, 0.11f), 0.1f);
            head.localEulerAngles = Vector3.Lerp(head.localEulerAngles, new Vector3(7.5f, 0f, 0f), 0.1f);
        }
        if(IsGrounded)
        {
            isFalling = false;
            ...
            //Debug.Log("Grounded");
            head.localPosition = Vector3.Lerp(head.localPosition, new Vector3(0f, 0.6f, 0.0f), 0.1f);
    ...
    private void FixedUpdate()
    {
        ...
        if (IsGrounded || ((Vector3.Dot(fps_Rigidbody.velocity, transform.up)) < 0.1)) {
            RaycastHit[] hits = Physics.RaycastAll(ray, capsule.height * jumpRayLength);
            float nearest = float.PositiveInfinity;
            IsGrounded = false;
            for(int i = 0; i < hits.Length; i++) {
                if (!hits[i].collider.isTrigger && hits[i].distance < nearest && (hits[i].transform.tag == "LevelParts" || hits[i].transform.tag == "Slider Block")) {
                    IsGrounded = true;
        ...
        //Max fall speed;
        if (fps_Rigidbody.velocity.magnitude > 7)
        {
            fps_Rigidbody.velocity = 7f * Vector3.Normalize(fps_Rigidbody.velocity);
            //Debug.Log("lowering speed");
            //Debug.Log(fps_Rigidbody.velocity);
        }
        ...
        if (IsGrounded)
        {
            fps_Rigidbody.AddForce(Physics.gravity * (0.8f));
        }
        else
        {
    ...
    public void LateUpdate()
    {
        ...
        {
            //portalRotation(Quaternion.Euler(new Vector3(0f, 0f, 90f)));
            //fps_Rigidbody.velocity = new Vector3(0f, 4f, 0f);
