/// Orientation Correction by Marnix
/// Should be able to control the controller Orientation and Gravity of the entire scene
/// 
...
public class Backup : MonoBehaviour
{
    ...
    public Transform rotationMarker;
...
public class BETA_SETTINGS{
    ...
    private void Awake()
    {
        ...
        rotationMarker.position = transform.position;
        rotationMarker.rotation = transform.localRotation;
    ...
    public void portalRotation(Quaternion rotation)
    {
        originalRotation = originalRotation * rotation;
    ...
    private void FixedUpdate()
    {
        ...
        if (IsGrounded || (Quaternion.Inverse(originalRotation) * fps_Rigidbody.velocity).y < 0.1)
        {
        ...
        {
            if (Physics.Raycast(new Vector3(transform.position.x, transform.position.y - 0.75f, transform.position.z + 0.1f), -transform.up, out advanced.surfaceAngleCheck, 1f))
            {
                ...
                if (Vector3.Angle(advanced.surfaceAngleCheck.normal, transform.up) < 89)
                {
                    ...
                    else if (Vector3.Angle(advanced.surfaceAngleCheck.normal, transform.up) > 44)
                    {
                        advanced.tooSteep = true;
                        isSprinting = false;
                        dMove = (transform.forward * inputXY.y * speed + transform.right * inputXY.x) + -4f * transform.up;
                    }
                }
            }
        ...
        if (true) //playerCanMove
        {
            float correctionAngle = Vector2.Angle(rotationMarker.up, transform.up);
            Quaternion correction = Quaternion.Normalize(Quaternion.AngleAxis(correctionAngle, Vector3.Cross(rotationMarker.up, transform.up)));
        ...
        if (false)
        { //fOVKick.useFOVKick && wasWalking == isSprinting && fps_Rigidbody.velocity.magnitude > 0.1f && !isCrouching
            StopAllCoroutines();
        ...
        if (false) //dynamicFootstep.useDynamicFootStepProcess
        {
    ...
    public void LateUpdate()
    {
        ...
        {
            //portalRotation(new Vector3(0f, 0f, 90f));
        }
    }
